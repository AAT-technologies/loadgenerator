pipeline {
  ...
  stages {
    stage ('Testing') {
      steps {
          git branch: 'dev', credentialsId: 'for-git', url: 'https://github.com/AAT-technologies/loadgenerator.git'
          sh ''' sudo docker login -u delalixx -p dckr_pat_-dfSKHYHBVZNLTVX1R5sxmNGJwo
          '''
          sh ''' sudo docker system prune -af
          '''
         
         sh ''' cd app-load/loadgenerator
                   ls
                   sudo docker build -t delalixx/loadgenerator:v1 .
                   sudo docker push delalixx/loadgenerator:v1
                   '''
         sh ''' sudo docker system prune -af
                  '''
